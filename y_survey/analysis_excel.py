...
def getFileList(import_dir, wildcard, recursion):
    os.chdir(import_dir)
    ...
    for name in files:
        fullname = os.path.join(import_dir, name)
...
def importData(path):
    ...
    file_list = getFileList(path, wildcard, 1)
    ...
    time = file_list[1]
    # 省份
    province = file_list[2]
    # #文件类型
    FileType = file_list[3]
